...
class DynamoUtilities:
    def __init__(self, access_key_id, secret_access_key, region, session_key=None):
    ...
    def scan_data_single_eq_filter(self, table_name, key_name, value):
        result = []
        try:
            table_obj = self.dynamo_resource.Table(table_name)
            response = table_obj.scan(FilterExpression=Attr(key_name).eq(value))
            if "Items" in response:
                result.extend(response["Items"])
            while "LastEvaluatedKey" in response:
                response = table_obj.scan(ExclusiveStartKey=response['LastEvaluatedKey'], FilterExpression=Attr(key_name).eq(value))
                result.extend(response["Items"])
        except Exception as e:
            print("Error: {0}\nException: {1}".format(e, traceback.format_exc()))
        return result
    ...
    def incremental_update_data(self, table_name, condition_dict, attr_name, frequency_value):
        status = False
        try:
            table_obj = self.dynamo_resource.Table(table_name)
            table_obj.update_item(Key=condition_dict,
                                  UpdateExpression="set {0} = {0} + :val".format(attr_name),
    ...
    def delete_single_data(self, table_name, key_name, key_value):
        status = False
        try:
            table_obj = self.dynamo_resource.Table(table_name)
            table_obj.delete_item(Key={key_name: key_value})
            status = True
        except Exception as e:
            print("Error: {0}\nException: {1}".format(e, traceback.format_exc()))
        return status

    def decremental_update_data(self, table_name, condition_dict, attr_name, frequency_value):
        status = False
        try:
            table_obj = self.dynamo_resource.Table(table_name)
            table_obj.update_item(Key=condition_dict,
    ...
    def update_single_data(self, table_name, condition_dict, attr_name, attr_value):
        status = False
        try:
            table_obj = self.dynamo_resource.Table(table_name)
            table_obj.update_item(Key=condition_dict,
                                  UpdateExpression="set {0}=:value".format(attr_name),
                                  ExpressionAttributeValues={ ":value": attr_value
                                  })
            status = True
        except Exception as e:
            print("Error: {0}\nException: {1}".format(e, traceback.format_exc()))
