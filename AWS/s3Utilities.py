...
class S3Utilities:
    ...
    def generate_presigned_url_download(self, bucket_name, file_key, timeout=10*60):
        url = None
        try:
            url = self.s3_client.generate_presigned_url('get_object', Params={'Bucket': bucket_name, 'Key': file_key}, ExpiresIn=timeout)
        except Exception as e:
            print("{0}\n{1}".format(e, traceback.format_exc()))
        return url
